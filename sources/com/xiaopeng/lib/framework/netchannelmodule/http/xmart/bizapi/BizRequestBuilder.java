...
public class BizRequestBuilder {
    ...
    private String generateSignature() {
        ...
        try {
            String a2 = TextUtils.isEmpty(this.mBody) ? "" : d.a(this.mBody);
            Mac mac = Mac.getInstance("HmacSHA256");
            byte[] bytes = this.mSecretKey.getBytes(XmartV1Constants.UTF8_ENCODING);
            mac.init(new SecretKeySpec(bytes, 0, bytes.length, "HmacSHA256"));
            c.a(TAG, "generateSignature origin:" + (value + '&' + signParameters + '&' + a2));
            byte[] doFinal = mac.doFinal((value + '&' + signParameters + '&' + a2).getBytes(XmartV1Constants.UTF8_ENCODING));
    ...
    private String generateXpClient() {
        String a2 = b.d.b.e.m.c.a();
        String replaceAll = TextUtils.isEmpty(a2) ? "" : a2.toUpperCase().replaceAll(" ", "");
        StringBuilder sb = new StringBuilder();
    ...
    private String getAuthorization() {
        if (this.mNeedAuthorizationInfo) {
            ...
            try {
                ...
                c.f(TAG, "getAuthorization fail : " + e);
                return null;
            }
        }
        return null;
    ...
    private String getNetworkType() {
        int a2 = e.a(GlobalConfig.getApplicationContext());
        return a2 != 1 ? a2 != 2 ? a2 != 3 ? a2 != 10 ? NETWORK_TYPE_UNKNOWN : NETWORK_TYPE_WIFI : NETWORK_TYPE_4G : NETWORK_TYPE_3G : NETWORK_TYPE_2G;
    }
    ...
    private static String getSignParameters(a aVar, String str) {
        ...
        ArrayList arrayList = new ArrayList(aVar.f1238d.keySet());
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            if (queryParameterNames.contains(str2) || str2.startsWith(BizConstants.HEADER_PREFIX)) {
                if (aVar.f1238d.containsKey(str2)) {
                    if (b.g() && !TextUtils.isEmpty(parse.getQueryParameter(str2))) {
                        throw new RuntimeException("Duplicate keys in header and parameters!");
                    }
                    queryParameter = aVar.f1238d.get(str2);
